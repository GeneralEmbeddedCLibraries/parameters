...
/**
 *  Device Parameters on set callback
 */
typedef void (*pf_par_on_set_cb_t)(const par_num_t par_num, const par_type_t new_val, const par_type_t old_val);

typedef struct par_on_set_cb
{
    const pf_par_on_set_cb_t callback;      /**<Callback function pointer */
    ...
    struct par_on_set_cb **  next;          /**<Pointer to next callback block */
} par_on_set_cb_t;

#define PAR_DEFINE_ON_SET_CB(name, par, cb)         \
    static const par_on_set_cb_t name =             \
    {                                               \
        .callback      = cb,                        \
        .par_num_first = par,                       \
        .is_range      = false,                     \
        .next          = &(par_on_set_cb_t*){NULL}, \
    }

#define PAR_DEFINE_ON_SET_RANGE_CB(name, par_first, par_last, cb)   \
    static const par_on_set_cb_t name =                             \
    {                                                               \
        .callback      = cb,                                        \
